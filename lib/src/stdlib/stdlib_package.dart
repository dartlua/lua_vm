...
int preloadSearcher(LuaState ls) {
	...
	if (ls.getField(-1, name) == LuaType.nil) { /* not found? */
...
int luaSearcher(LuaState ls) {
  ...
  if (path == null) {
	...
	if (ls.loadFile(filename) == LuaStatus.ok) { /* module loaded successfully? */
...
void _findLoader(LuaState ls, String name) {
	...
	for (var i = 1; ; i++) {
		if (ls.rawGetI(3, i) == LuaType.nil) { /* no more searchers? */
			ls.pop(1);         /* remove nil */
...
class LuaStdlibPackage {
  ...
  int pkgRequire(LuaState ls) {
  	...
  	if (ls.getField(2, name) == LuaType.nil) { /* module set no value? */
